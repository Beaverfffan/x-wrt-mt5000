...
define Device/bananapi_bpi-r4-common
  ...
  ARTIFACTS := \
	       ...
	       sdcard.img.gz sdcard_8g.img.gz emmc-gpt.img.gz \
	       snand-preloader.bin snand-bl31-uboot.fip
  ARTIFACT/emmc-gpt.bin		:= mt798x-gpt emmc
  ARTIFACT/emmc-preloader.bin	:= mt7988-bl2 emmc-comb
...
define Device/ruijie_rg-x60
  ...
  DEVICE_MODEL := RG-X60
  DEVICE_DTS := mt7986a-ruijie-rg-x60 mt7986a-ruijie-rg-x60-pro
  DEVICE_DTS_DIR := ../dts
  DEVICE_DTS_CONFIG := config@ruijie_x60_gsw_en8811h_phy config@ruijie_x60_gsw
  UBINIZE_OPTS := -E 5
  BLOCKSIZE := 128k
  PAGESIZE := 2048
  IMAGE_SIZE := 64512k
  KERNEL_IN_UBI := 1
  DEVICE_PACKAGES := kmod-mt7915e kmod-mt7986-firmware mt7986-wo-firmware airoha-en8811h-firmware kmod-phy-airoha-en8811h
  SUPPORTED_DEVICES += ruijie,rg-x60-pro
  KERNEL = kernel-bin | lzma | \
	fit lzma $$(KDIR)/image-$$(word 1,$$(DEVICE_DTS)).dtb $$(KDIR)/image-$$(word 2,$$(DEVICE_DTS)).dtb
  KERNEL_INITRAMFS = kernel-bin | lzma | \
	fit lzma $$(KDIR)/image-$$(word 1,$$(DEVICE_DTS)).dtb $$(KDIR)/image-$$(word 2,$$(DEVICE_DTS)).dtb with-initrd
  IMAGE/sysupgrade.bin := sysupgrade-tar | append-metadata
endef
...
define Device/tenda_ax12-pro-v2
  DEVICE_VENDOR := Tenda
  DEVICE_MODEL := AX12 Pro v2
  DEVICE_ALT0_VENDOR := Tenda
  DEVICE_ALT0_MODEL := RX12 Pro v2
  DEVICE_ALT1_VENDOR := Tenda
  DEVICE_ALT1_MODEL := TX12 Pro v2
  DEVICE_DTS := mt7981b-tenda-ax12-pro-v2
  DEVICE_DTS_DIR := ../dts
  DEVICE_PACKAGES := kmod-mt7915e kmod-mt7981-firmware mt7981-wo-firmware uboot-envtools
  KERNEL_LOADADDR := 0x40000000
  KERNEL_INITRAMFS := kernel-bin | lzma | \
  ...
  IMAGE/sysupgrade.bin := append-kernel | pad-to 128k | append-rootfs | pad-rootfs | check-size | append-metadata
endef
TARGET_DEVICES += tenda_ax12-pro-v2
